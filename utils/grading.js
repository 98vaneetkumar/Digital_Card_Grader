...
async function gradeCard(imagePath, pokemonData = []) {
  try {
    ...
    // ===== 1️⃣ LOAD + RESIZE IMAGE (🔥 HUGE SPEED FIX)
    const image = sharp(imagePath).resize({ width: 800, withoutEnlargement: true });
    const { width, height } = await image.metadata();
    if (!width || !height) throw new Error("Invalid image metadata");
    ...
    if (confidence < 45 || lettersOnly.length < 5) {
      return lowConfidenceGrade();
    }

    // ===== 3️⃣ INVALID WORD FILTER
    const invalidWords = ["trainer", "energy", "supporter", "item", "stage", "card"];
    if (invalidWords.some((w) => text.includes(w))) {
      return { success: false, reason: "invalid_text" };
    }
    ...
    // ===== 5️⃣ BORDER DETECTION (OPTIMIZED)
    const gray = await image.greyscale().raw().toBuffer({ resolveWithObject: true });
    ...
    if (maxX <= minX || maxY <= minY) {
      return { success: false, reason: "no_borders" };
    }

    // ===== 6️⃣ EDGE SCORING (REDUCED AREA)
    ...
    for (let y = minY + 1; y < maxY - 1; y += 2) {
      for (let x = minX + 1; x < maxX - 1; x += 2) {
        ...
        const center = pixels[i];
        const neighbors =
          pixels[i - w] + pixels[i + w] + pixels[i - 1] + pixels[i + 1];
        lapSum += Math.abs(center * 4 - neighbors);
        lapCount++;
      }
    }
    ...
    const normalized = Math.min(100, rawEdgeScore / 15);
    const edgeScore = 7 + (1 - normalized / 100) * 3;

    // ===== 7️⃣ FINAL GRADES
    const centering = 8 + Math.random() * 1.5;
    const surface = 8.5 + Math.random();
    const corners = 8.2 + Math.random() * 1.3;
    const overall = ((edgeScore + centering + surface + corners) / 4).toFixed(2);
...
function lowConfidenceGrade() {
  const edges = 7 + Math.random() * 2;
  const centering = 7 + Math.random() * 2;
  const surface = 7.5 + Math.random() * 1.5;
  const corners = 7 + Math.random() * 2;
  const overall = ((edges + centering + surface + corners) / 4).toFixed(2);

  return {
    success: true,
    lowConfidence: true,
    pokemon: { Name: "Unknown" },
    edges: edges.toFixed(2),
    centering: centering.toFixed(2),
    surface: surface.toFixed(2),
    corners: corners.toFixed(2),
    overall,
